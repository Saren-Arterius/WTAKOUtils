...
public class ArgCountEntity {
    ...
    public ArgCountEntity(final CommandSender sender, final String[] args) {
        ...
        if (args.length >= 3) {
            ...
            if (world == null) {
                world = sender instanceof Player ? ((Player) sender).getWorld() : Main.getInstance().getServer()
                        .getWorlds().get(0);
            }
        }
        ...
        int counter = 0;
        sender.sendMessage(MessageFormat.format(Lang.COUNT_ENTITY_TOP_CHUNKS.toString(), listItems, world.getName(),
                entityType != null ? entityType.name() : Lang.COUNT_ENTITY_ENTITIES.toString()));
